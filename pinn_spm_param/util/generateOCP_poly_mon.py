...
def objective(p):
    return ((polynomial(p) - yd) ** 2).sum() + 1e1 * constraint_1st_der(
        p
    ).sum()
...
def polyOpt(order):
    ...
    res = minimize(objective, x0=output.beta[::-1], method="SLSQP")
    return res


def iterativeOpt(order_list):
    error = []
    printProgressBar(
        0,
        len(order_list) - 1,
...
bestOrd = iterativeOpt(list(range(30)))
res = polyOpt(bestOrd)
pars = res.x
poly = np.poly1d(pars)
